...
export function DashboardPage() {
  ...
  const profitLabel = group?.settlementMode === 'WinnerKeepsLoserPays' ? t('dashboard.stats.netLoss') : t('dashboard.stats.netProfit');
    ...
    <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-200 mt-1">
              {t('dashboard.welcomeBack')} <span className="text-emerald-400 font-semibold">{user?.displayName}</span>
            </p>
          </div>
      ...
      {!isLoading && (
        <div className="space-y-6">
          {/* Stats Cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard label={t('dashboard.stats.myGroups')} value={groups?.length ?? 0} Icon={GroupsIcon} bg={STAT_CARDS[0].bg} hoverBg={STAT_CARDS[0].hoverBg} onClick={goTo('/groups')} />
            <StatCard label={t('dashboard.stats.upcoming')} value={upcomingMatches.length} Icon={SportsSoccerIcon} bg={STAT_CARDS[1].bg} hoverBg={STAT_CARDS[1].hoverBg} onClick={goTo('/matches')} />
            <StatCard label={t('dashboard.stats.winRate')} value={`${stats.winRate}%`} Icon={WinRateIcon} bg={STAT_CARDS[2].bg} hoverBg={STAT_CARDS[2].hoverBg} onClick={goTo('/bets')} />
            <StatCard
              label={profitLabel}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-bold text-emerald-700 uppercase tracking-wide">{t('dashboard.sections.upcomingMatches')}</h2>
                </div>
                <button
              onClick={goTo('/matches')}
                ...
                >
                  {t('common.viewAll')} <ArrowIcon sx={{ fontSize: 14 }} />
                </button>
              </div>
              <div className="divide-y divide-gray-50">
                {upcomingMatches.length === 0 ? (
                  <div className="flex flex-col items-center py-12 text-slate-400">
                    ...
                    <p className="text-sm">{t('dashboard.empty.noUpcoming')}</p>
                  </div>
                ) : (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-bold text-sky-700 uppercase tracking-wide">{t('dashboard.sections.recentBets')}</h2>
                </div>
                <button
                  onClick={goTo('/bets')}
              ...
              <div className="divide-y divide-gray-50">
                {!bets?.length ? (
                  <div className="flex flex-col items-center py-12 text-slate-400">
                    ...
                    <p className="text-sm">{t('dashboard.empty.noBets')}</p>
                    <p className="text-xs mt-1">{t('dashboard.empty.noBetsHint')}</p>
                  </div>
                ) : (
                  recentBets.map((bet) => {
                    ...
                    return (
                      <div key={bet.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
                        ...
                        <div className="flex flex-col items-center min-w-[60px]">
                          <span className="text-[10px] font-medium text-gray-400 uppercase mb-0.5">{t('common.profitLoss')}</span>
