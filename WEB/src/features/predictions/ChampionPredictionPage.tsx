  ...
  return (
    <div>
      ...
      {config.isSettled && winnerTeam && (
        <div className="mb-6 p-5 rounded-2xl bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 flex items-center gap-4">
          <div className="w-14 h-14 rounded-full bg-emerald-100 flex items-center justify-center ring-4 ring-emerald-200">
            {winnerTeam.flagUrl ? (
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-bold text-emerald-800 uppercase tracking-wide">Championship Settled</p>
            </div>
            <p className="text-xl font-black text-emerald-900 mt-0.5">
              {winnerTeam.name} <span className="text-emerald-600 font-medium text-base">({winnerTeam.code})</span>
            </p>
          </div>
        </div>
      )}
      ...
      {myPrediction && (
        <div className="mb-6 p-4 rounded-2xl bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-violet-500 uppercase tracking-wide">Your Prediction</p>
                <p className="text-lg font-bold text-gray-900">{myPrediction.selectedTeamName}</p>
                {config.isSettled && (
        ...
        >
          ...
          {groupPredictions && groupPredictions.length > 0 && (
            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
              activeTab === 'leaderboard' ? 'bg-amber-100 text-amber-700' : 'bg-gray-200 text-gray-500'
            }`}>
